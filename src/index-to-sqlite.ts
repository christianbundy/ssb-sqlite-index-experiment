...
let main = async () => {
  ...
  let updateDescriptionSt = db.prepare(
    `UPDATE authors SET description = ? WHERE key = ?`
  );
  ...
  let ingestMsg = (msg: SSBMessage): void => {
    if (msg.value.previous === null) {
      insertAuthorSt.run(msg.value.author);
    }
    ...
    if (msg.value.content.type === "about") {
      ...
      if (
        ...
        content.about === msg.value.author
      ) {
  ...
  let numAuthors = db.prepare("SELECT COUNT(*) as count FROM authors;").get()
    .count;
